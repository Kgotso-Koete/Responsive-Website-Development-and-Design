//playground.js
// Attribution: https://github.com/chidimo/Music-Machine/tree/master/public
...
cymbal = new Maxim();
snaredrum = new Maxim();
hi_hat = new Maxim(); // create hi hat object
...
hi_hat_Player = hi_hat.loadFile("/hihat2.wav");
hi_hat_Player.loop
...
stopOrPlay_hi_hat = function(volume) {
  hi_hat_Player.volume(volume)
}
...
Template.playground.helpers({
    ...
    "snaredrum" : function() {
      ...
      var starter = MusicMachine.findOne();
      if (starter) {
        Session.set('hi_hat', starter.hi_hat)
        stopOrPlay_hi_hat(starter.hi_hat)
      }
      return Session.get('hi_hat') 
    },
...
Template.playground.events({
    ...
    "click .js-control_hi_hat": function () {
      var c = Session.get('hi_hat');
      var mach = MusicMachine.findOne({});

      if (c === 0){
          Session.set('hi_hat', 1);
          MusicMachine.update({_id:mach._id}, {$set: {hi_hat: 1}});
      } else {
          Session.set('hi_hat', 0);
          MusicMachine.update({_id:mach._id}, {$set: {hi_hat: 0}});
      }
  },
